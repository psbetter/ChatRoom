...
//            System.out.println("[Error] 读取注册用户信息失败！");
            DataBuffer.serverLog.append("[Error] 读取注册用户信息失败！\n");
            e.printStackTrace();
        }finally{
            IOUtil.close(ois);
        }
    ...
    private void saveAllUser(List<User> users) {
        ...
        try {
            ...
            DataBuffer.serverLog.append("[Error] 保存注册用户信息失败！\n");
            e.printStackTrace();
        }finally{
            IOUtil.close(oos);
        }
    ...
    public void initUser(){
        DataBuffer.serverLog.append("[Error] UserService的user初始化\n");
